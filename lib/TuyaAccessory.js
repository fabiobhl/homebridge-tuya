    ...
    _change(data) {
        ...
        Object.keys(data).forEach(key => {
            ...
            if (isNonEmptyPlainObject(changes)) {
                if (changes.hasOwnProperty('51')) {
                    try {
                        const buf = Buffer.from(changes['51'], 'base64');
                        const hex = buf.toString('hex');
                        let sceneInfo = '';
                        if (buf.length >= 8) {
                            const sceneId = buf.readUInt16BE(0);
                            const hue = buf.readUInt16BE(2);
                            const sat = buf.readUInt16BE(4) / 10;
                            const bri = buf.readUInt16BE(6) / 10;
                            sceneInfo = ` scene=${sceneId} hue=${hue} sat=${sat}% bri=${bri}%`;
                        }
                        this.log.info(`[TuyaLan] [${this.context.name}] dp51 change raw=${changes['51']} hex=${hex}${sceneInfo}`);
                    } catch (ex) {
                        this.log.warn(`[TuyaLan] [${this.context.name}] dp51 change raw=${changes['51']} (decode failed: ${ex && ex.message})`);
                    }
                }
