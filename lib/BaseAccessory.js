class BaseAccessory {
    ...
    convertColorTemperatureFromHomeKitToTuya(value) {
        ...
        if (this.device.context.linearWhiteColor) {
            const clampedValue = Math.min(600, Math.max(0, value));
            const range = max - min || 1;
            const ratio = clampedValue / 600;
            const convertedValue = Math.round(min + (range * (1 - ratio)));
    ...
    convertColorTemperatureFromTuyaToHomeKit(value) {
        ...
        if (this.device.context.linearWhiteColor) {
            ...
            const range = max - min || 1;
            const ratio = (max - clampedValue) / range;
            const convertedValue = Math.round(ratio * 600);
            return Math.min(600, Math.max(0, convertedValue));
        }
