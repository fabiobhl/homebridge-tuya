...
class RGBTWLightAccessory extends BaseAccessory {
    ...
    _getTintedColorForTemperature(value) {
        ...
        if (!this.hasModeDP) {
            ...
            const warmReference = this.convertHomeKitColorTemperatureToHomeKitColor(600);
            color.h = 50;
            const targetSaturation = Math.round(Math.max(0, Math.min(100, warmReference.s * 0.6)));
            color.s = targetSaturation;
        }
        return color;
    }
