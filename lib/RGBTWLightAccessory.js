...
class RGBTWLightAccessory extends BaseAccessory {
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (!this.hasModeDP) {
                ...
                if (changes.hasOwnProperty(this.dpColorTemperature)) {
                    ...
                    this._logTintState(newColorTemperature, changes[this.dpColorTemperature], 'dpColorTemperature change');
                ...
                if (changes.hasOwnProperty(this.dpColor)) {
                    ...
                    this.log.info(`[TuyaLan] [${this.device.context.name}] dpColor change raw=${changes[this.dpColor]} decoded=${JSON.stringify(newColor)}`);
            ...
            switch (state[this.dpMode]) {
                case this.cmdWhite:
                    ...
                    if (changes.hasOwnProperty(this.dpColorTemperature) && this.convertColorTemperatureFromHomeKitToTuya(characteristicColorTemperature.value) !== changes[this.dpColorTemperature]) {
                        ...
                        this._logTintState(newColorTemperature, state[this.dpColorTemperature], 'dpColorTemperature change');
                        const newColor = this._getTintedColorForTemperature(newColorTemperature);
    ...
    _logTintState(homeKitValue, rawValue, context) {
        if (typeof homeKitValue !== 'number') return;
        const coolCt = 140;
        const warmCt = 600;
        const clampedValue = Math.max(coolCt, Math.min(warmCt, homeKitValue));
        const range = warmCt - coolCt || 1;
        const ratio = (clampedValue - coolCt) / range;
        // this.log.info(`[TuyaLan] [${this.device.context.name}] ${context} hk=${homeKitValue} raw=${rawValue} ratio=${ratio.toFixed(3)}`);
    }
    ...
    _getTintedColorForTemperature(value) {
        ...
        const currentTuyaCt = this.device.state ? this.device.state[this.dpColorTemperature] : undefined;
        const currentHomeKitCt = typeof currentTuyaCt === 'number'
            ? this.convertColorTemperatureFromTuyaToHomeKit(currentTuyaCt)
            : 'n/a';

        // this.log.info(
        //     `[TuyaLan] [${this.device.context.name}] tint ratio=${ratio.toFixed(3)} requested=${value} current=${currentHomeKitCt}`
        // );
        ...
        if (!this.hasModeDP) {
            ...
            const coolTargetHue = this.device.context.coldPresetHue || 51;
            const coolTargetSaturation = this.device.context.coldPresetSaturation || 51;
            const warmTargetHue = this.device.context.warmPresetHue || 35;
            const warmTargetSaturation = this.device.context.warmPresetSaturation || 80;

            const hueDelta = ((warmTargetHue - coolTargetHue + 540) % 360) - 180;
            const interpolatedHue = (coolTargetHue + hueDelta * ratio + 360) % 360;
            const interpolatedSaturation = coolTargetSaturation + (warmTargetSaturation - coolTargetSaturation) * ratio;

            color.h = Math.round(interpolatedHue);
            color.s = Math.round(Math.max(0, Math.min(100, interpolatedSaturation)));
        }
        return color;
    }
    ...
    setColorTemperature(value, callback) {
        ...
        const tuyaColorTemperature = this.convertColorTemperatureFromHomeKitToTuya(value);
        // this.log.info(`[TuyaLan] [${this.device.context.name}] setColorTemperature resolved=${value} (${tuyaColorTemperature})`);
        ...
        const payload = this.hasModeDP
            ? {[this.dpMode]: this.cmdWhite, [this.dpColorTemperature]: tuyaColorTemperature}
            : {
                [this.dpColorTemperature]: tuyaColorTemperature,
