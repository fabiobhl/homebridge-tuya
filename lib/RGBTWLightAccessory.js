...
class RGBTWLightAccessory extends BaseAccessory {
    ...
    _getTintedColorForTemperature(value) {
        ...
        if (!this.hasModeDP) {
            ...
            const coolReference = this.convertHomeKitColorTemperatureToHomeKitColor(coolCt);
            const warmReference = this.convertHomeKitColorTemperatureToHomeKitColor(warmCt);
            ...
            if (blendRatio > 0) {
                ...
                const hueDelta = ((warmTargetHue - baseHue + 540) % 360) - 180;
                const blendedHue = (baseHue + hueDelta * blendRatio + 360) % 360;
                ...
                color.s = Math.round(Math.max(0, Math.min(100, blendedSaturation)));
            } else {
                color.h = coolReference.h;
                color.s = coolReference.s;
            }
        }
        return color;
    }
